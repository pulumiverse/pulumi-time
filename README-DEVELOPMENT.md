...
We showcase a Pulumi-owned provider based on an upstream provider named `terraform-provider-time`.  Substitute appropriate values below for your use case.
...
* `provider/` contains the Go code used to create the provider as well as generate the SDKs in the various languages that Pulumi supports.
  * `provider/cmd/pulumi-tfgen-time` generates the Pulumi resource schema (`schema.json`), based on the Terraform provider's resources.
  * `provider/cmd/pulumi-resource-time` generates the SDKs in all supported languages from the schema, placing them in the `sdk/` folder.
...
1. Create the schema by running the following command:
    ...
    ```text
    warning: resource time_something not found in provider map; skipping
    warning: resource time_something_else not found in provider map; skipping
    warning: data source time_something not found in provider map; skipping
    warning: data source time_something_else not found in provider map; skipping
    ```
...
1. **Add resource mappings:** For each resource in the provider, add an entry in the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "time_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "time_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "time_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "time_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
1. Copy the `pulumi-resource-time` binary generated by `make provider` and place it in your `$PATH` (`$GOPATH/bin` is a convenient choice), e.g.:

    ```bash
    cp bin/pulumi-resource-time $GOPATH/bin
    ```
